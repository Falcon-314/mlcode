...
class MLPDataset_train:
    ...
    def __getitem__(self, idx):
        dct = {
            'x' : torch.tensor(self.features[idx, :].reshape(1,5,3), dtype=torch.float),
...
class cnn_2d(nn.Module):
    def __init__(self):
        ...
        self.dense1 = nn.Conv2d(1, 8, kernel_size=3, stride=1, padding=0, bias=True)
        
        self.flatten = nn.Flatten()
        
        self.dense2 = nn.Linear(24, 12)
    ...
    for epoch in range(CFG.epochs):
        ...
        if score > best_score:
            ...
            torch.save({'model': model.state_dict(), 
                        'preds': preds},
                        OUTPUT_DIR+f'cnn_2d_fold{fold}_best.pth')
    
    check_point = torch.load(OUTPUT_DIR+f'cnn_2d_fold{fold}_best.pth')
...
def main():
    ...
    if CFG.inference:      
        # inference
        model = cnn_2d()
        states = [torch.load(OUTPUT_DIR+f'cnn_2d_fold{fold}_best.pth') for fold in CFG.trn_fold]
